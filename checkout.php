...
$select="SELECT * FROM cart WHERE use_rid='$id_u'";
...
<?php
                  $select="SELECT * FROM product WHERE id='$id_bro'";
                  $result=$conn->query($select);
                      $proo=$result->fetch_assoc();
  
?>
  
                    <li class="border-bottom my-2"></li>
                    <li class="d-flex align-items-center justify-content-between"><strong class="small font-weight-bold"><?=$proo['name']?></strong><span class="text-muted small">$<?php 
                    $pr=($proo['price']*$row['quan']);
                    echo $pr;
                    $total+=$pr;
